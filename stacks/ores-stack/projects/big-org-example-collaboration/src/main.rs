...
impl TypedModule for WorkModule {
    ...
    const NAME: &'static str = "big-org-example-collaboration-work";
...
fn main() -> std::io::Result<()> {
    ...
    eprintln!("big-org-example-collaboration listening on {bind}");

    for stream in listener.incoming() {
        let mut stream = stream?;
        let mut request = [0_u8; 4096];
        let _ = stream.read(&mut request);
        let body = r#"{"ok":true,"scenario":"collaboration","stack":"ores-stack"}"#;
