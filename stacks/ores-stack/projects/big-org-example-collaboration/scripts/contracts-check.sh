#!/usr/bin/env bash
set -euo pipefail
PROJECT="$(cd "$(dirname "$0")/.." && pwd)"; ROOT="$(cd "$PROJECT/../../../.." && pwd)"
TJSV="$ROOT/.local/bin/tjsv"; if [[ ! -x "$TJSV" ]]; then TJSV="$ROOT/node_modules/.bin/tjsv"; fi
test -x "$TJSV" || exit 2; mkdir -p .artifacts/conformance
