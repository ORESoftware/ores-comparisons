...
impl TypedModule for WorkModule {
    ...
    const NAME: &'static str = "big-org-example-commerce-work";
    ...
    async fn handle(&self, _context: &(), input: String) -> Result<String, String> {
        Ok(format!("{{\"ok\":true,\"scenario\":\"commerce\",\"input\":{input:?}}}"))
    }
}
...
fn main() -> std::io::Result<()> {
    ...
    eprintln!("big-org-example-commerce listening on {bind}");

    for stream in listener.incoming() {
        let mut stream = stream?;
        let mut request = [0_u8; 4096];
        let _ = stream.read(&mut request);
        let body = r#"{"ok":true,"scenario":"commerce","stack":"ores-stack"}"#;
        write!(
            stream,
