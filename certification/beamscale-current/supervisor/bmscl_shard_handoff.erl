...
%% Shared safety contract for moving a virtual shard between replica sets.
%%
%% This module deliberately models only the handoff barrier. Placement policy
%% (for example weighted rendezvous hashing) decides where a shard should move;
%% storage/consensus decides how bytes are replicated. The handoff state machine
%% prevents authority from advancing until every target replica has caught up to
%% the frozen committed position.
%%
%% Queue users normally interpret barrier_position as a replicated-log commit
%% index. Durable-lock users may interpret it as a compact replicated-state or
%% consensus commit position. The history retention policy is intentionally not
%% shared between those products.
